...
async function main() {
  ...
  const GameReporter = await ethers.getContractFactory("GameReporter");
  const reporter = await GameReporter.deploy();
  console.log(verifyStr, process.env.HARDHAT_NETWORK, reporter.address);
}
